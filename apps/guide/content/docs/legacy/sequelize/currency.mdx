...
<Callout type="error" title="Attention! Security Risk!">
    **Make sure you use version 5 or later of Sequelize!** As used in this guide, version 4 and earlier will pose a security threat. You can read more about this issue on the [Sequelize issue tracker](https://github.com/sequelize/sequelize/issues/7310).
</Callout>
...
Reflect.defineProperty(Users.prototype, 'addItem', {
	value: async (item) => {
		const userItem = await UserItems.findOne({ // [!code word:findOne]
...
Reflect.defineProperty(Users.prototype, 'getItems', {
	value: () => {
		return UserItems.findAll({ // [!code word:findAll]
...
async function addBalance(id, amount) { // [!code ++:18]
