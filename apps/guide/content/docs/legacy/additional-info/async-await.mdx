...
<Callout>
	This example uses ES6 code. If you do not know what that is, you should read up on that [here](/guide/legacy/additional-info/es6-syntax).
</Callout>

```js
function deleteMessages(amount) {
	return new Promise((resolve, reject) => { // [!code word:Promise]
...
```js
async function declaredAsAsync() { // [!code word:async]
	// ...
}
```
...
```js
const declaredAsAsync = async () => { // [!code word:async]
	// ...
};
```
...
client.on(Events.InteractionCreate, (interaction) => {
	// ...
	if (commandName === 'react') { // [!code focus:7]
