...
client.once(Events.ClientReady, () => { // [!code word:=>]
	console.log('Ready!');
});
...
```js
client.once(Events.ClientReady, function () { // [!code --:3]
	console.log('Ready!');
});
client.once(Events.ClientReady, () => console.log('Ready!')); // [!code ++]

client.on(Events.TypingStart, function (typing) { // [!code --:3]
...
client.on(Events.MessageCreate, function (message) { // [!code --:3]
...
The code is shorter and looks cleaner, but it shouldn't be necessary if you follow along with the [command handler](/guide/legacy/app-creation/handling-commands) part of the guide.
...
<Callout>
	The underscore `_` prefix is a convention for unused variables. Some lint rules will error or warn if you define identifiers without using them in your code but ignore identifiers starting with `_`.
</Callout>
