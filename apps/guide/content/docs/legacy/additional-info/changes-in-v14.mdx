...
@discord-selfbot-sdk/bot v14 makes the switch to Discord API v10!
...
In addition, the old enums exported by @discord-selfbot-sdk/bot v13 and lower are replaced with new enums from [discord-api-types](https://discord-api-types.dev/api/discord-api-types-v10).

#### New enum differences

Most of the difference between enums from @discord-selfbot-sdk/bot and discord-api-types can be summarized as so:
...
```js
const { Client, Intents } = require('@discord-selfbot-sdk/bot'); // [!code --]
const { Client, GatewayIntentBits, Partials } = require('@discord-selfbot-sdk/bot'); // [!code ++]
...
```js
const { ApplicationCommandType, ApplicationCommandOptionType } = require('@discord-selfbot-sdk/bot'); // [!code ++]
...
```js
const { ButtonStyle } = require('@discord-selfbot-sdk/bot'); // [!code ++]
...
```js
const { ChannelType } = require('@discord-selfbot-sdk/bot'); // [!code ++]
...
```js
const { Constants } = require('@discord-selfbot-sdk/bot'); // [!code --]
const { Colors } = Constants; // [!code --]
const { Colors } = require('@discord-selfbot-sdk/bot'); // [!code ++]
```
...
const { ButtonBuilder } = require('@discord-selfbot-sdk/bot'); // [!code ++]
...
This REST event has been removed as @discord-selfbot-sdk/bot now uses [Undici](https://github.com/nodejs/undici) as the underlying request handler. You must now use a [Diagnostics Channel](https://undici.nodejs.org/#/docs/api/DiagnosticsChannel). Here is a simple example:
...
Other functions in `Util` have been moved to top-level exports so you can directly import them from `@discord-selfbot-sdk/bot`.

```js
import { Util } from '@discord-selfbot-sdk/bot'; // [!code --]
Util.escapeMarkdown(message); // [!code --]
import { escapeMarkdown } from '@discord-selfbot-sdk/bot'; // [!code ++]
...
```js
const { ComponentType } = require('@discord-selfbot-sdk/bot'); // [!code ++]
...
@discord-selfbot-sdk/bot uses `@discordjs/ws` internally
