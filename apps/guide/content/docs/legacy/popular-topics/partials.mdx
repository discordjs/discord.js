...
As we said earlier, partials do not have all the information necessary to make them fully functional @discord-selfbot-sdk/bot structures, so it would not be a good idea to enable the functionality by default. Users should know how to handle them before opting into this feature.
...
```js
const { Client, Partials } = require('@discord-selfbot-sdk/bot');
