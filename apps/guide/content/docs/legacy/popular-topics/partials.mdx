...
One example leveraging partials is the handling of reactions on uncached messages, which is explained on [this page](/guide/legacy/popular-topics/reactions#listening-for-reactions-on-old-messages).
...
<Callout type="warn">
	Partial structures are enabled **globally**. You cannot make them work for only a specific event or cache. You very **likely need to adapt** other parts of your code that are accessing data from the relevant caches. All caches holding the respective structure type might return partials as well!
</Callout>
...
```js
if (message.partial) { // [!code word:partial]
	message
		.fetch()
		.then((fullMessage) => {
