...
<Callout>
	Webhook fetching will always make use of collections and Promises. If you do not understand either concept, revise them, and then come back to this section. You can read about collections [here](/guide/legacy/additional-info/collections), and Promises [here](/guide/legacy/additional-info/async-await) and [here](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Using_promises).
</Callout>
...
```js
channel
	.createWebhook({ // [!code word:createWebhook]
		name: 'Some-username',
...
webhookClient.send({ // [!code word:send]
...
```js
const message = await webhook.editMessage('123456789012345678', { //[!code word:editMessage]
	content: 'Edited!',
	embeds: [embed],
});
```
