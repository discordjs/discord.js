...
```js
const { PermissionsBitField } = require('@discord-selfbot-sdk/bot');
...
```js
const { ChannelType, PermissionsBitField } = require('@discord-selfbot-sdk/bot');
...
@discord-selfbot-sdk/bot provides a `toArray()` method, which can be used to convert a `Permissions` object into an array containing permission flags. This is useful if you want to display/list them and it enables you to use other array manipulation methods. For example:
...
Some methods and properties in @discord-selfbot-sdk/bot return permission decimals rather than a Permissions object, making it hard to manipulate or read them if you don't want to use bitwise operations.
