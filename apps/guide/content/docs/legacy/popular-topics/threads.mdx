...
<Callout>
    You can use the `ThreadChannel#isThread` type guard to make sure a channel is a `ThreadChannel`!
</Callout>
...
```js
const thread = await channel.threads.create({ // [!code word:create]
	name: 'food-talk',
...
```js
const thread = await message.startThread({ // [!code word:startThread]
	name: 'food-talk',
	autoArchiveDuration: ThreadAutoArchiveDuration.OneHour,
	reason: 'Needed a separate thread for food',
});
...
```
<Callout>
	The created thread and the message it originated from will share the same ID.
</Callout>
...
<Callout>
	You cannot create private threads on an existing message.
</Callout>
