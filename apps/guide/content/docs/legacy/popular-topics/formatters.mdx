...
@discord-selfbot-sdk/bot provides the `formatters` package which contains a variety of utilities you can use when writing your Discord bot.
...
```js
const { hyperlink, hideLinkEmbed } = require('@discord-selfbot-sdk/bot');
...
```js
const { inlineCode, codeBlock } = require('@discord-selfbot-sdk/bot');
...
```js
const { time, TimestampStyles } = require('@discord-selfbot-sdk/bot');
const date = new Date();
...
```js
const { channelMention, roleMention, userMention } = require('@discord-selfbot-sdk/bot');
