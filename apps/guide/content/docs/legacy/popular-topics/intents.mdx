...
Gateway Intents were introduced by Discord so bot developers can choose which events their bot receives based on which data it needs to function. Intents are named groups of pre-defined WebSocket events, which @discord-selfbot-sdk/bot client will receive. If you omit `DirectMessageTyping`, for example, you will no longer receive typing events from direct messages. If you do not specify intents, discord.js will throw an error.
...
You can find the list of all current gateway intents and the events belonging to each on the [Discord API documentation](https://discord.com/developers/docs/topics/gateway#list-of-intents) and the enum values used in @discord-selfbot-sdk/bot on the [Discord API types documentation](https://discord-api-types.dev/api/discord-api-types-v10/enum/GatewayIntentBits).

- If you need your bot to receive messages (`MESSAGE_CREATE` - `"messageCreate"` in @discord-selfbot-sdk/bot), you need the `Guilds` and `GuildMessages` intent, plus the `MessageContent` privileged intent to receive the `content`, `attachments`, `embeds` and `components` fields of the message.
- If you want your bot to post welcome messages for new members (`GUILD_MEMBER_ADD` - `"guildMemberAdd"` in @discord-selfbot-sdk/bot), you need the `GuildMembers` privileged intent, and so on.

```js
const { Client, GatewayIntentBits } = require('@discord-selfbot-sdk/bot');
...
@discord-selfbot-sdk/bot provides the utility structure `IntentsBitField` to simplify the modification of intents bitfields.
...
```js
const { Client, IntentsBitField } = require('@discord-selfbot-sdk/bot');
...
In @discord-selfbot-sdk/bot, Permissions and Intents bitfields are represented as either the decimal value of said bit field or its referenced flags. Every position in a permissions bitfield represents one of these flags and its state (either referenced `1` or not referenced `0`).
