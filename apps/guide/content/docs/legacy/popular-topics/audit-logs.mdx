...
Let's start by glancing at the `Guild#fetchAuditLogs` method and how to work with it. Like many discord.js methods, it returns a [`Promise`](/guide/legacy/additional-info/async-await) containing the `GuildAuditLogs` object. This object has one property, `entries`, which holds a [`Collection`](/guide/legacy/additional-info/collections) of `GuildAuditLogsEntry` objects, and consequently, the information you want to retrieve.
