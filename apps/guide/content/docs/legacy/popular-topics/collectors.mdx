...
`Collector` are useful to enable your bot to obtain *additional* input after the first command was sent. An example would be initiating a quiz, where the bot will "await" a correct response from somebody.
...
For more information on working with modals, see the [modals section of this guide](/guide/legacy/interactions/modals).
