...
Modals are pop-up forms that allow you to prompt users for additional input. This form-like interaction response blocks the user from interacting with Discord until the modal is submitted or dismissed. In this section, we will cover how to create, show, and receive modals using @discord-selfbot-sdk/bot!
...
With the `ModalBuilder` class, @discord-selfbot-sdk/bot offers a convenient way to build modals step by step using setters and callbacks.
...
```js
const { Events, ModalBuilder } = require('@discord-selfbot-sdk/bot');
...
```js
const { LabelBuilder, ModalBuilder } = require('@discord-selfbot-sdk/bot');
...
```js
const { LabelBuilder, ModalBuilder, TextInputBuilder, TextInputStyle } = require('@discord-selfbot-sdk/bot');
