...
<Callout>
	The `customId` is a developer-defined string of up to 100 characters. This field is the unique identify of the modal
...
<Callout type="warn">
	`label` can be a max length of 45 characters

    `description` can be a max length of 100 characters
...
client.on(Events.InteractionCreate, async (interaction) => {
	...
	if (interaction.commandName === 'ping') {
		...
		const hobbiesInput = new TextInputBuilder()
			...
			// Uninteractable text inside of the text input box
		...
		const hobbiesLabel = new LabelBuilder()
			...
			.setDescription('Activities you like to participate in')
			// [!code ++:2]
			// Set text input component of the label
