...
Radio Group is an interactive component for selecting one option from a list of up to ten options.
...
client.on(Events.InteractionCreate, async (interaction) => {
	...
	if (interaction.commandName === 'ping') {
		...
		const radioGroupLabel = new LabelBuilder().setLabel('Radio Group').setRadioGroupComponent((radioGroup) =>
			radioGroup.setCustomId('checkboxes').addOptions([
...
```js
const radioGroupLabel = new FileUploadBuilder()
	// Set the optional identifier for component
	.setId(1)
...
Checkbox Group is an interactive component for selecting one to ten options.
...
client.on(Events.InteractionCreate, async (interaction) => {
	...
	if (interaction.commandName === 'ping') {
		...
		// [!code focus:13]
		// [!code ++:9]
		const checkBoxesLabel = new LabelBuilder().setLabel('CheckBox Group').setCheckboxGroupComponent((checkBoxes) =>
...
```js
const checkBoxesLabel = new CheckboxGroupBuilder()
	// Set the optional identifier for component
	.setId(1)
...
client.on(Events.InteractionCreate, async (interaction) => {
	...
	if (interaction.commandName === 'ping') {
		...
		modal.addLabelComponents(radioGroupLabel, checkBoxesLabel);
		// [!code ++:1]
		modal.addLabelComponents(radioGroupLabel, checkBoxesLabel, checkBoxLabel);
...
A Checkbox component can be customized to apply validation via the following `CheckBoxGroupBuilder` method:

```js
const checkBoxesLabel = new CheckboxBuilder()
	// Set the optional identifier for component
	.setId(1);
```
