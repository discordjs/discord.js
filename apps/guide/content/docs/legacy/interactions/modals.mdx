...
<Callout>
    This page is a follow-up to the [interactions (slash commands) page](/guide/legacy/slash-commands/advanced-creation). Please carefully read that section first, so that you can understand the methods used in this section.
</Callout>
...
<Callout>
    You can have a maximum of five `ActionRowBuilder`s per modal builder, and one `TextInputBuilder` within an `ActionRowBuilder`. Currently, you can only use `TextInputBuilder`s in modal action rows builders.
</Callout>
...
For a detailed guide on receiving message components via collectors, please refer to the [collectors guide](/guide/legacy/popular-topics/collectors#interaction-collectors).
...
<Callout>
    If you're using typescript, you can use the `ModalSubmitInteraction#isFromMessage` typeguard, to make sure the received interaction was from a `MessageComponentInteraction`.
</Callout>
