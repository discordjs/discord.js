...
Node.js uses an event-driven architecture, making it possible to execute code when a specific event occurs. The @discord-selfbot-sdk/bot library takes full advantage of this. You can visit the `Client` documentation to see the full list of events.
...
const { Events } = require('@discord-selfbot-sdk/bot');
...
const { Events, MessageFlags } = require('@discord-selfbot-sdk/bot');
...
const { Client, Collection, GatewayIntentBits } = require('@discord-selfbot-sdk/bot');
...
<Callout>
	In most cases, you can access your `client` instance in other files by obtaining it from one of the other
	@discord-selfbot-sdk/bot structures, e.g. `interaction.client` in the `interactionCreate` event. You do not need to
	manually pass it to your events.
</Callout>
