...
To use environment variables in Node.js, we recommend you use the command line interface flag `--env-file` to point to the `.env` file you want to use. Note that the file name `.env` is just a convention. You could for example have a `development.env` and `production.env` file with different values depending on the Discord application you want to run your code.
