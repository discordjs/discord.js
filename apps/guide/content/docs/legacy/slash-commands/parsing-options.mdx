...
If you specified preset choices for your String, Integer, or Number option, getting the selected choice is exactly the same as the free-entry options above. Consider the [gif command](/guide/legacy/slash-commands/advanced-creation#choices) example you looked at earlier:
...
The snippet below uses the same `info` command from the [subcommand creation guide](/guide/legacy/slash-commands/advanced-creation#subcommands) to demonstrate how you can control the logic flow when replying to different subcommands:
