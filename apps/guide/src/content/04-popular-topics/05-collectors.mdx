...
<p>
	<DocsLink type="class" parent="Collector">
		<code>Collector</code>s
	</DocsLink>{' '}
	are useful to enable your bot to obtain *additional* input after the first command was sent. An example would be
	initiating a quiz, where the bot will "await" a correct response from somebody.
</p>
