...
try {
	const collected = message.awaitReactions({ filter: collectorFilter, max: 4, time: 60000, errors: ['time'] });
...
try {
	const interaction = await message.awaitMessageComponent({ filter: collectorFilter, componentType: ComponentType.StringSelect, time: 60000 });
...
```js
try {
	const interaction = initialInteraction.awaitModalSubmit({ time: 60_000, filter });
