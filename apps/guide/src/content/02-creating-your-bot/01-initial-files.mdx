...
We'll start by creating a _`package.json`_ file. This file will keep track of the dependencies your project uses as well as other information. This can be done by running the following command in your project directory:
...
```sh yarn
yarn init -y # You must go into your package.json file and add "type": "module"
```
...
Once you're done with that, on to the next step!
...
<Alert title="Caution" type="danger">
	If you're using Git, don't commit this file and instead [ignore it via
	_`.gitignore`_](/creating-your-bot/#git-and-gitignore).
</Alert>

## Using environment variables

Environment variables are special values for your environment (e.g., a terminal session, a Docker container, or an environment variable file). You can pass these values into your code's scope so that you can use them.
...
You can access the set values in your code via the _`process.env`_ global variable, accessible in any file. Note that values passed this way will always be strings, and you might need to parse them to a number if using them to do calculations.
...
Open your code editor and create a new file. We suggest that you save the file as _`index.ts`_ or _`index.js`_, depending on whether you use TypeScript. However, you are free to name it whatever you want.
