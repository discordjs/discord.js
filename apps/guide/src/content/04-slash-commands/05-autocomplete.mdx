...
To use autocomplete with your commands, _instead_ of listing static choices, the option must be set to use autocompletion. This is done by setting `_autocomplete: true_` in the <DocsLink type="typedef" parent="ApplicationCommandOptionData" />. Autocomplete can be used on the same option types that support static choices; _`String`_, _`Integer`_ and _`Number`_.
