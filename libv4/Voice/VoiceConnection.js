...
var VoiceConnection = function (_EventEmitter) {
	...
	function VoiceConnection(channel, client, session, token, server, endpoint) {
		...
		_this.token = token;
		_this.server = server;
		_this.endpoint = endpoint.split(":")[0];
		_this.vWS = null; // vWS means voice websocket
		_this.ready = false;
		_this.vWSData = {};
		_this.encoder = new _AudioEncoder2.default();
		_this.udp = null;
		...
		_this.volume = new _VolumeTransformer2.default();
		_this.paused = false;
		_this.init();
		return _this;
	}
	...
	_createClass(VoiceConnection, [{
		...
		value: function playStream(stream) {
			var channels = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 2;
		...
		value: function playFile(stream) {
			...
			var callback = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : function (err, str) {};
			...
			return new Promise(function (resolve, reject) {
				_this2.encoder.encodeFile(stream, options).then(function (data) {
		...
		value: function playRawStream(stream) {
			...
			options.volume = options.volume !== undefined ? options.volume : this.getVolume();
			return new Promise(function (resolve, reject) {
				_this3.encoder.encodeStream(stream, options).then(function (data) {
					self.streamProc = data.proc;
		...
		key: "playArbitraryFFmpeg",
		value: function playArbitraryFFmpeg(ffmpegOptions, options) {
			var _this4 = this;
			...
			options.volume = options.volume !== undefined ? options.volume : this.getVolume();
			return new Promise(function (resolve, reject) {
				_this4.encoder.encodeArbitraryFFmpeg(ffmpegOptions, options).then(function (data) {
					self.streamProc = data.proc;
					self.instream = data.instream;
					var intent = self.playStream(data.stream);
					resolve(intent);
		...
		value: function init() {
			...
			_dns2.default.lookup(this.endpoint, function (err, address, family) {
				var vWS = self.vWS = new _ws2.default("wss://" + _this5.endpoint, null, { rejectUnauthorized: false });
				_this5.endpoint = address;
					...
					switch (data.op) {
						...
						case 4:
							if (data.d.secret_key && data.d.secret_key.length > 0) {
								...
								for (var i = 0; i < _this5.secret.length; i++) {
		...
		key: "wrapVolume",
		value: function wrapVolume(stream) {
