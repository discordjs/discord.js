...
export class Base {
  ...
  public toJSON(...props: Record<string, boolean | string>[]): object;
...
export class Collection<K, V> extends BaseCollection<K, V> {
  public toJSON(): (V extends { toJSON(): infer R } ? R : Flatten<V>)[];
}
...
export class Util {
  ...
  public static flatten<T>(obj: T, ...props: Record<string, boolean | string>[]): Flatten<T>;
...
export type APIMessageContentResolvable = string | number | boolean | bigint | symbol | readonly StringResolvable[];
...
export type ExplicitContentFilterLevel = 'DISABLED' | 'MEMBERS_WITHOUT_ROLES' | 'ALL_MEMBERS';
...
export type Flatten<T> = T extends object ? object : T;
...
export interface GuildChannelCloneOptions extends GuildCreateChannelOptions {
  name?: string;
}
...
export type MembershipStates = 'INVITED' | 'ACCEPTED';
...
export type MessageTarget = TextChannel | NewsChannel | DMChannel | User | GuildMember | Webhook | WebhookClient;
...
export type SystemMessageType = Exclude<MessageType, 'DEFAULT' | 'REPLY'>;

export type TargetUser = number;
...
export type WebhookTypes = 'Incoming' | 'Channel Follower';
