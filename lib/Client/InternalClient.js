...
var InternalClient = (function () {
	...
	InternalClient.prototype.login = function login(email, password, tfaCode) {
		var _this11 = this;
		...
		if (!this.tokenCacher.done) {
			return new Promise(function (resolve, reject) {
				setTimeout(function () {
					_this11.login(email, password, tfaCode).then(resolve)["catch"](reject);
				}, 20);
			});
		} else {
		...
		return this.apiRequest("post", _Constants.Endpoints.LOGIN, false, {
			...
			if (res.mfa) {
				if (!tfaCode) return Promise.reject(new Error("no two factor authentication code provided, even though activated!"));

				_this11.client.emit("debug", "detected two factor authentication");

				return _this11.apiRequest("post", _Constants.Endpoints.TOTP, false, {
					ticket: res.ticket,
					code: tfaCode
				}).then(function (res) {
					var token = res.token;
