...
class InternalClient {
				...
				return new Promise((resolve, reject) => {
					...
					var check = data => {
						if (data.t === "VOICE_SERVER_UPDATE") {
							...
							var chan = new _VoiceConnection2.default(channel, this.client, session, token, server, endpoint);
	...
	sendWebhookMessage(webhook, _content, options = {}) {
		return this.resolver.resolveWebhook(webhook).then(destination => {
			...
			return this.apiRequest("post", `${ _Constants.Endpoints.WEBHOOK_MESSAGE(destination.id, destination.token) }${ slack ? "/slack" : "" }?wait=true`, true, options).catch(console.error).then(res => destination.channel.messages.add(new _Message2.default(res, destination.channel, this.client)));
		});
	}
