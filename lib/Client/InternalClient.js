...
var InternalClient = (function () {
	...
	InternalClient.prototype.deleteMessage = function deleteMessage(_message) {
		...
		return chain.then(function () {
			return _this14.apiRequest("del", _Constants.Endpoints.CHANNEL_MESSAGE(message.channel.id, message.id), true);
		}).then(function () {
	...
	InternalClient.prototype.updateMessage = function updateMessage(msg, _content) {
		...
		return this.apiRequest("patch", _Constants.Endpoints.CHANNEL_MESSAGE(message.channel.id, message.id), true, {
			...
			return message.channel.messages.update(message, new _StructuresMessage2["default"](res, message.channel, _this15.client));
		});
	};
	...
	InternalClient.prototype.sendFile = function sendFile(where, _file, name) {
		...
		return this.resolver.resolveChannel(where).then(function (channel) {
			return _this16.resolver.resolveFile(_file).then(function (file) {
				return _this16.apiRequest("post", _Constants.Endpoints.CHANNEL_MESSAGES(channel.id), true, null, {
					name: name,
					file: file
				}).then(function (res) {
	...
	InternalClient.prototype.getBans = function getBans(server) {
		...
		return this.apiRequest("get", _Constants.Endpoints.SERVER_BANS(server.id), true).then(function (res) {
			return res.map(function (ban) {
				return _this18.users.add(new _StructuresUser2["default"](ban.user, _this18.client));
			});
		});
	};
	...
	InternalClient.prototype.createChannel = function createChannel(server, name) {
		...
		return this.apiRequest("post", _Constants.Endpoints.SERVER_CHANNELS(server.id), true, {
			...
			var channel;
			if (res.type === "text") {
				channel = new _StructuresTextChannel2["default"](res, _this19.client, server);
			} else {
				channel = new _StructuresVoiceChannel2["default"](res, _this19.client, server);
			}
			return server.channels.add(_this19.channels.add(channel));
		});
	};
	...
	InternalClient.prototype.deleteChannel = function deleteChannel(_channel) {
		...
		return this.resolver.resolveChannel(_channel).then(function (channel) {
			return _this20.apiRequest("del", _Constants.Endpoints.CHANNEL(channel.id), true).then(function () {
				channel.server.channels.remove(channel);
				_this20.channels.remove(channel);
			});
		});
	};
	...
	InternalClient.prototype.moveMember = function moveMember(user, channel) {
		...
		return this.resolver.resolveChannel(channel).then(function (channel) {
			...
			if (channel.type !== "voice") {
				...
				return _this21.apiRequest("patch", _Constants.Endpoints.SERVER_MEMBERS(server.id) + "/" + user.id, true, { channel_id: channel.id }).then(function (res) {
	...
	InternalClient.prototype.createRole = function createRole(server, data) {
		...
		return this.apiRequest("post", _Constants.Endpoints.SERVER_ROLES(server.id), true).then(function (res) {
			var role = server.roles.add(new _StructuresRole2["default"](res, server, _this22.client));

			if (data) {
				return _this22.updateRole(role, data);
			}
			return role;
		});
	};
	...
	InternalClient.prototype.updateRole = function updateRole(role, data) {
		...
		return this.apiRequest("patch", _Constants.Endpoints.SERVER_ROLES(server.id) + "/" + role.id, true, newData).then(function (res) {
			return server.roles.update(role, new _StructuresRole2["default"](res, server, _this23.client));
		});
	};
	...
	InternalClient.prototype.getInvite = function getInvite(invite) {
		...
		return this.apiRequest("get", _Constants.Endpoints.INVITE(invite), true).then(function (res) {
			if (!_this27.channels.has("id", res.channel.id)) {
				return new _StructuresInvite2["default"](res, null, _this27.client);
			}
			return _this27.apiRequest("post", _Constants.Endpoints.CHANNEL_INVITES(res.channel.id), true, { validate: invite }).then(function (res2) {
				return new _StructuresInvite2["default"](res2, _this27.channels.get("id", res.channel.id), _this27.client);
			});
		});
	};
			...
			if (server) {
				return this.apiRequest("get", _Constants.Endpoints.SERVER_INVITES(server.id), true).then(function (res) {
					return res.map(function (data) {
						return new _StructuresInvite2["default"](data, _this28.channels.get("id", data.channel.id), _this28.client);
					});
				});
			}
		}
		return this.resolver.resolveChannel(channel).then(function (channel) {
			return _this28.apiRequest("get", _Constants.Endpoints.CHANNEL_INVITES(channel.id), true).then(function (res) {
				return res.map(function (data) {
					return new _StructuresInvite2["default"](data, _this28.channels.get("id", data.channel.id), _this28.client);
				});
			});
	...
	InternalClient.prototype.overwritePermissions = function overwritePermissions(channel, role, updated) {
		...
		return this.resolver.resolveChannel(channel).then(function (channel) {
			...
			if (role instanceof _StructuresUser2["default"]) {
				...
				role = _this29.resolver.resolveRole(role);
			}
			...
			return _this29.apiRequest("put", _Constants.Endpoints.CHANNEL_PERMISSIONS(channel.id) + "/" + data.id, true, data);
		});
	};
	...
	InternalClient.prototype.sendTyping = function sendTyping(channel) {
		...
		return this.resolver.resolveChannel(channel).then(function (channel) {
			return _this30.apiRequest("post", _Constants.Endpoints.CHANNEL(channel.id) + "/typing", true);
		});
	};
	...
	InternalClient.prototype.startTyping = function startTyping(channel) {
		...
		return this.resolver.resolveChannel(channel).then(function (channel) {
			...
			_this31.intervals.typing[channel.id] = setInterval(function () {
				return _this31.sendTyping(channel)["catch"](function (error) {
					return _this31.emit("error", error);
				});
			}, 4000);

			return _this31.sendTyping(channel);
		});
	};
	...
	InternalClient.prototype.stopTyping = function stopTyping(channel) {
		...
		return this.resolver.resolveChannel(channel).then(function (channel) {
			...
			clearInterval(_this32.intervals.typing[channel.id]);
			_this32.intervals.typing[channel.id] = false;
		});
	};
			...
			switch (packet.t) {
				...
				case _Constants.PacketType.SERVER_MEMBERS_CHUNK:
					...
					if (server) {
						...
						for (var _iterator7 = data.members, _isArray7 = Array.isArray(_iterator7), _i7 = 0, _iterator7 = _isArray7 ? _iterator7 : _iterator7[Symbol.iterator]();;) {
							var _ref7;
