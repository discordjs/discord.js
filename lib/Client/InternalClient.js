...
var InternalClient = (function () {
	...
	InternalClient.prototype.getServerWebhooks = function getServerWebhooks(server) {
		...
		return this.apiRequest("get", _Constants.Endpoints.SERVER_WEBHOOKS(server.id), true).then(function (res) {
			...
			return res.map(function (webhook) {
				return server.webhooks.add(new _StructuresWebhook2["default"](webhook, server, _this36.channels.get("id", webhook.channel_id), _this36.users.get("id", webhook.user.id)));
			});
		});
	};
	...
	InternalClient.prototype.sendWebhookMessage = function sendWebhookMessage(webhook, _content) {
		...
		return this.resolver.resolveWebhook(webhook).then(function (destination) {
			var content = _this38.resolver.resolveString(_content);

			if (_this38.client.options.disableEveryone || options.disableEveryone) {
				content = content.replace(/(@)(everyone|here)/g, "$1​$2");
			}

			if (!options.hasOwnProperty("username")) {
				options.username = _this38.user.username;
			}
			...
			return _this38.apiRequest("post", "" + _Constants.Endpoints.WEBHOOK_MESSAGE(destination.id, destination.token) + (slack ? "/slack" : "") + "?wait=true", true, options)["catch"](console.error).then(function (res) {
				return destination.channel.messages.add(new _StructuresMessage2["default"](res, destination.channel, _this38.client));
			});
		});
	};
	...
	InternalClient.prototype.createWS = function createWS(url) {
		...
		this.websocket.onclose = function (event) {
			_this39.websocket = null;
			_this39.state = _ConnectionState2["default"].DISCONNECTED;
			if (event && event.code) {
				_this39.client.emit("warn", "WS close: " + event.code);
				var err;
				if (event.code === 4001) {
				...
				if (err) {
					_this39.client.emit("error", err);
				}
			}
			_this39.disconnected(_this39.client.options.autoReconnect);
		};

		this.websocket.onerror = function (e) {
			_this39.client.emit("error", e);
			_this39.websocket = null;
			_this39.state = _ConnectionState2["default"].DISCONNECTED;
			_this39.disconnected(_this39.client.options.autoReconnect);
		};
		...
		this.websocket.onmessage = function (e) {
			...
			_this39.client.emit("raw", packet);

			if (packet.s) {
				_this39.sequence = packet.s;
			}

			switch (packet.op) {
				case 0:
					_this39.processPacket(packet);
					break;
				case 1:
					_this39.heartbeat();
					break;
				case 7:
					_this39.disconnected(true);
					break;
				case 9:
					_this39.sessionID = null;
					_this39.sequence = 0;
					_this39.identify();
					break;
				case 10:
					if (_this39.sessionID) {
						_this39.resume();
					} else {
						_this39.identify();
					}
					_this39.heartbeat();
					_this39.intervals.kai = setInterval(function () {
						return _this39.heartbeat();
					}, packet.d.heartbeat_interval);
					break;
				case 11:
					_this39.heartbeatAcked = true;
					break;
				default:
					_this39.client.emit("unknown", packet);
					break;
			}
		};
	};
			...
			case _Constants.PacketType.READY:
				...
				data.guilds.forEach(function (server) {
					if (!server.unavailable) {
						server = _this40.servers.add(new _StructuresServer2["default"](server, client));
						if (client.options.bot === false) {
							_this40.unsyncedGuilds++;
							_this40.syncGuild(server.id);
						}
						if (_this40.client.options.forceFetchUsers && server.members && server.members.length < server.memberCount) {
							_this40.getGuildMembers(server.id, Math.ceil(server.memberCount / 1000));
						}
					} else {
						client.emit("debug", "server " + server.id + " was unavailable, could not create (ready)");
						_this40.unavailableServers.add(server);
					}
				});
				data.private_channels.forEach(function (pm) {
					_this40.private_channels.add(new _StructuresPMChannel2["default"](pm, client));
				});
				if (!data.user.bot) {
					...
					data.relationships.forEach(function (friend) {
						if (friend.type === 1) {
							// is a friend
							_this40.friends.add(new _StructuresUser2["default"](friend.user, client));
						} else if (friend.type === 2) {
							// incoming friend requests
							_this40.blocked_users.add(new _StructuresUser2["default"](friend.user, client));
						} else if (friend.type === 3) {
							// incoming friend requests
							_this40.incoming_friend_requests.add(new _StructuresUser2["default"](friend.user, client));
						} else if (friend.type === 4) {
							// outgoing friend requests
							_this40.outgoing_friend_requests.add(new _StructuresUser2["default"](friend.user, client));
						} else {
