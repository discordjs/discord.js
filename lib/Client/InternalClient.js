...
var InternalClient = (function () {
	...
	InternalClient.prototype.createWS = function createWS(url) {
		...
		this.websocket.onclose = function (event) {
			_this38.websocket = null;
			_this38.state = _ConnectionState2["default"].DISCONNECTED;
			if (event && event.code) {
				_this38.client.emit("warn", "WS close: " + event.code);
				var err;
				if (event.code === 4001) {
				...
				if (err) {
					_this38.client.emit("error", err);
				}
			}
			_this38.disconnected(_this38.client.options.autoReconnect);
		};

		this.websocket.onerror = function (e) {
			_this38.client.emit("error", e);
			_this38.websocket = null;
			_this38.state = _ConnectionState2["default"].DISCONNECTED;
			_this38.disconnected(_this38.client.options.autoReconnect);
		};
		...
		this.websocket.onmessage = function (e) {
			...
			_this38.client.emit("raw", packet);

			if (packet.s) {
				_this38.sequence = packet.s;
			}

			switch (packet.op) {
				case 0:
					_this38.processPacket(packet);
					break;
				case 1:
					_this38.heartbeat();
					break;
				case 7:
					...
					break;
				case 9:
					_this38.sessionID = null;
					_this38.sequence = 0;
					_this38.identify();
					break;
				case 10:
					if (_this38.sessionID) {
						_this38.resume();
					} else {
						_this38.identify();
					}
					_this38.heartbeat();
					_this38.intervals.kai = setInterval(function () {
						return _this38.heartbeat();
					}, packet.d.heartbeat_interval);
					break;
				case 11:
					_this38.heartbeatAcked = true;
					break;
				default:
					_this38.client.emit("unknown", packet);
					break;
			}
		};
	};
			...
			case _Constants.PacketType.READY:
				...
				data.guilds.forEach(function (server) {
					if (!server.unavailable) {
						server = _this39.servers.add(new _StructuresServer2["default"](server, client));
						if (client.options.bot === false) {
							_this39.unsyncedGuilds++;
							_this39.syncGuild(server.id);
						}
						if (_this39.client.options.forceFetchUsers && server.members && server.members.length < server.memberCount) {
							_this39.getGuildMembers(server.id, Math.ceil(server.memberCount / 1000));
						}
					} else {
						client.emit("debug", "server " + server.id + " was unavailable, could not create (ready)");
						_this39.unavailableServers.add(server);
					}
				});
				data.private_channels.forEach(function (pm) {
					_this39.private_channels.add(new _StructuresPMChannel2["default"](pm, client));
				});
				if (!data.user.bot) {
					...
					data.relationships.forEach(function (friend) {
						if (friend.type === 1) {
							// is a friend
							_this39.friends.add(new _StructuresUser2["default"](friend.user, client));
						} else if (friend.type === 2) {
							// incoming friend requests
							_this39.blocked_users.add(new _StructuresUser2["default"](friend.user, client));
						} else if (friend.type === 3) {
							// incoming friend requests
							_this39.incoming_friend_requests.add(new _StructuresUser2["default"](friend.user, client));
						} else if (friend.type === 4) {
							// outgoing friend requests
							_this39.outgoing_friend_requests.add(new _StructuresUser2["default"](friend.user, client));
						} else {
