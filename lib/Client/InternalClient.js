...
class InternalClient {
	...
	createServer(name, region = "london") {
		...
		return new Promise((resolve, reject) => {
			...
			request.post(Endpoints.SERVERS).set("authorization", self.token).send({ name, region }).end((err, res) => {
				if (err) {
	...
	joinServer(invite) {
		...
		return new Promise((resolve, reject) => {
			...
			if (invite) {
				request.post(Endpoints.INVITE(invite)).set("authorization", self.token).end((err, res) => {
					if (err) {
						reject(err);
					} else {
	...
	leaveServer(srv) {
		...
		return new Promise((resolve, reject) => {
			...
			if (server) {
				...
				request.del(Endpoints.SERVER(server.id)).set("authorization", self.token).end((err, res) => {
					if (err) {
						reject(new Error(err));
					} else {
	...
	login(email, password) {
		...
		return new Promise((resolve, reject) => {
			if (self.state === ConnectionState.DISCONNECTED || self.state === ConnectionState.IDLE) {
				...
				request.post(Endpoints.LOGIN).send({ email, password }).end(function (err, res) {
	...
	logout() {
		...
		return new Promise((resolve, reject) => {
			...
			request.post(Endpoints.LOGOUT).set("authorization", self.token).end((err, res) => {
				if (err) {
					reject(new Error(err));
				} else {
					if (this.websocket) {
	...
	sendMessage(where, _content, options = {}) {
		...
		return new Promise((resolve, reject) => {
			...
			self.resolver.resolveChannel(where).then(next).catch(e => reject(new Error("Error resolving destination - " + e)));
	...
	deleteMessage(_message, options = {}) {
		...
		return new Promise((resolve, reject) => {
			...
			if (message) {
				...
				function deleteMsg() {
					request.del(Endpoints.CHANNEL_MESSAGE(message.channel.id, message.id)).set("authorization", self.token).end((err, res) => {
						if (err) {
							reject(new Error(err));
						} else {
	...
	sendFile(where, _file, name = "image.png") {
		var self = this;
		return new Promise((resolve, reject) => {
			self.resolver.resolveChannel(where).then(next).catch(e => reject(new Error("couldn't resolve to channel - " + e)));

			function next(channel) {

				var file = self.resolver.resolveFile(_file);

				request.post(Endpoints.CHANNEL_MESSAGES(channel.id)).set("authorization", self.token).attach("file", file, name).end((err, res) => {
	...
	getChannelLogs(_channel, limit = 500, options = {}) {
		...
		return new Promise((resolve, reject) => {
			...
			self.resolver.resolveChannel(_channel).then(next).catch(e => reject(new Error("couldn't resolve to channel - " + e)));
			...
			function next(channel) {
				...
				request.get(`${ Endpoints.CHANNEL_MESSAGES(channel.id) }?limit=${ limit }${ joinedParams }`).set("authorization", self.token).end((err, res) => {
					if (err) {
						reject(new Error(err));
					} else {
						var logs = [];
	...
	getBans(server) {
		...
		return new Promise((resolve, reject) => {
			...
			request.get(`${ Endpoints.SERVER_BANS(server.id) }`).set("authorization", self.token).end((err, res) => {
				if (err) {
					reject(new Error(err));
				} else {
					var bans = [];
	...
	banMember(user, server, length = 1) {
		...
		return new Promise((resolve, reject) => {
			...
			request.put(`${ Endpoints.SERVER_BANS(server.id) }/${ user.id }/?delete-message-days=${ length }`).set("authorization", self.token).end((err, res) => {
				if (err) {
					reject(err);
				} else {
					resolve();
	...
	unbanMember(user, server) {
		...
		return new Promise((resolve, reject) => {
			...
			request.del(`${ Endpoints.SERVER_BANS(server.id) }/${ user.id }`).set("authorization", self.token).end((err, res) => {
				if (err) {
					reject(err);
				} else {
					resolve();
	...
	kickMember(user, server) {
		...
		return new Promise((resolve, reject) => {
			...
			request.del(`${ Endpoints.SERVER_MEMBERS(server.id) }/${ user.id }`).set("authorization", self.token).end((err, res) => {
				if (err) {
					reject(err);
				} else {
					resolve();
	...
	createRole(server, data) {
		...
		return new Promise((resolve, reject) => {
			...
			request.post(Endpoints.SERVER_ROLES(server.id)).set("authorization", self.token).end((err, res) => {
				if (err) {
					reject(err);
				} else {
	...
	deleteRole(role) {
		...
		return new Promise((resolve, reject) => {
			...
			request.del(Endpoints.SERVER_ROLES(role.server.id) + "/" + role.id).set("authorization", self.token).end((err, res) => {
				if (err) {
					reject(err);
				} else {
					resolve();
	...
	createInvite(chanServ, options) {
		...
		return new Promise((resolve, reject) => {
			...
			request.post(epoint).set("authorization", self.token).send(options).end((err, res) => {
				if (err) {
					reject(err);
				} else {
	...
	deleteInvite(invite) {
		...
		return new Promise((resolve, reject) => {
			...
			invite = self.resolver.resolveInviteID(invite);
			if (invite) {
				request.del(Endpoints.INVITE(invite)).set("authorization", self.token).end((err, res) => {
					if (err) {
						reject(err);
					} else {
						resolve();
	...
	sendTyping(channel) {
		...
		return new Promise((resolve, reject) => {
			...
			function next(channel) {
				...
				request.post(Endpoints.CHANNEL(channel.id) + "/typing").set("authorization", self.token).end((err, res) => {
					if (err) {
						reject(err);
					} else {
						resolve();
	...
	startTyping(channel) {
		...
		return new Promise((resolve, reject) => {
			...
			function next(channel) {
				...
				self.typingIntervals[channel.id] = setInterval(() => self.sendTyping(channel), 4000);
			}
		});
	}
	...
	ack(msg) {
		...
		return new Promise((resolve, reject) => {
			...
			if (msg) {
				...
				request.post(Endpoints.CHANNEL_MESSAGE(msg.channel.id, msg.id) + "/ack").set("authorization", self.token).end(err => {
					if (err) {
						reject(err);
					} else {
						resolve();
