...
var AudioEncoder = (function () {
	...
	AudioEncoder.prototype.encodeStream = function encodeStream(stream, options) {
		...
		return new Promise(function (resolve, reject) {
			...
			var enc = _child_process2["default"].spawn(self.getCommand(), ['-loglevel', '0', '-i', '-', '-f', 's16le', '-af', 'volume=' + volume, '-ar', '48000', '-ac', 2, 'pipe:1'], { stdio: ['pipe', 'pipe', 'ignore'] });
	...
	AudioEncoder.prototype.encodeFile = function encodeFile(file, options) {
		...
		return new Promise(function (resolve, reject) {
			var volume = options.volume;
			if (volume === undefined) volume = 1;
			var enc = _child_process2["default"].spawn(self.getCommand(), ['-loglevel', '0', '-i', file, '-f', 's16le', '-af', 'volume=' + volume, '-ar', '48000', '-ac', 2, 'pipe:1'], { stdio: ['pipe', 'pipe', 'ignore'] });
