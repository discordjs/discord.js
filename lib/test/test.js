...
function go() {
	var login = process.env.ds_token ? client.login(process.env.ds_token) : client.login(process.env.ds_email, process.env.ds_password);
...
function ready(token) {
	...
	console.log(client.servers.map(function (server) {
		return server.name;
	}));
