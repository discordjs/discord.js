...
import Readable from './VoiceReadable';
import FetchOpusEncoder from '../opus/OpusEngineList';
 ...
 */
export default class VoiceReceiver extends EventEmitter {
  ...
  handlePacket(msg, user) {
    ...
    if (this.listenerCount('pcm') > 0 || this.pcmStreams.size > 0) {
      if (!this.opusEncoders.get(user.id)) this.opusEncoders.set(user.id, FetchOpusEncoder);
