...
class ServerEmojiRoleManager {
  ...
  /**
   * The filtered collection of roles of the server emoji
   * @type {Collection<Snowflake, Role>}
   * @private
   * @readonly
   */
  get _roles() {
    return this.server.roles.cache.filter(role => this.emoji._roles.includes(role.id));
  }
  ...
  remove(roleOrRoles) {
    ...
    roleOrRoles = roleOrRoles.map(r => this.server.roles.resolveID(r));
   ...
   * serverEmoji.roles.set(['391156570408615936'])
   *   .then(console.log)
   *   .catch(console.error);
   * @example
   * // Remove all roles from an emoji
   * serverEmoji.roles.set([])
   *    .then(console.log)
   *    .catch(console.error);
   */
  set(roles) {
...
module.exports = ServerEmojiRoleManager;
