...
/**
 * Manages API methods for roles of a ServerMember and stores their cache.
 */
class ServerMemberRoleManager {
  constructor(member) {
    /**
     * The ServerMember this manager belongs to
     * @type {ServerMember}
     */
    this.member = member;
    /**
   ...
   */
  get _roles() {
    const everyone = this.server.roles.everyone;
    return this.server.roles.cache.filter(role => this.member._roles.includes(role.id)).set(everyone.id, everyone);
  }
  ...
  /**
   * The premium subscriber role of the server, if present on the member
   * @type {?Role}
   * @readonly
   */
  ...
  /**
   * The managed role this member created when joining the server, if any
  ...
  async add(roleOrRoles, reason) {
    if (roleOrRoles instanceof Collection || Array.isArray(roleOrRoles)) {
      roleOrRoles = roleOrRoles.map(r => this.server.roles.resolve(r));
      ...
      roleOrRoles = this.server.roles.resolve(roleOrRoles);
      if (roleOrRoles === null) {
        throw new TypeError('INVALID_TYPE', 'roles', 'Role, Snowflake or Array or Collection of Roles or Snowflakes');
      }

      await this.client.api.servers[this.server.id].members[this.member.id].roles[roleOrRoles.id].put({ reason });
  ...
  async remove(roleOrRoles, reason) {
    if (roleOrRoles instanceof Collection || Array.isArray(roleOrRoles)) {
      ...
      await this.client.api.servers[this.server.id].members[this.member.id].roles[roleOrRoles.id].delete({ reason });
   ...
   * serverMember.roles.set(['391156570408615936'])
   *   .then(console.log)
   *   .catch(console.error);
   * @example
   * // Remove all the roles from a member
   * serverMember.roles.set([])
...
module.exports = ServerMemberRoleManager;
