...
/**
 * Represents a server channel from any of the following:
 * - {@link TextChannel}
 * - {@link VoiceChannel}
...
class ServerChannel extends Channel {
  /**
   * @param {Server} server The server the server channel is part of
   * @param {Object} data The data for the server channel
   */
   ...
   */
  get parent() {
    return this.server.channels.cache.get(this.parentID) || null;
  }
   ...
   */
  get position() {
    const sorted = this.server._sortedChannels(this);
    return sorted.array().indexOf(sorted.get(this.id));
  }

  /**
   * Gets the overall set of permissions for a member or role in this channel, taking into account channel overwrites.
   * @param {ServerMemberResolvable|RoleResolvable} memberOrRole The member or role to obtain the overall
   * permissions for
   * @returns {?Readonly<Permissions>}
   */
  permissionsFor(memberOrRole) {
    const member = this.server.members.resolve(memberOrRole);
    if (member) return this.memberPermissions(member);
    const role = this.server.roles.resolve(memberOrRole);
    if (role) return this.rolePermissions(role);
    return null;
  }

  overwritesFor(member, verified = false, roles = null) {
    if (!verified) member = this.server.members.resolve(member);
    if (!member) return [];
    ...
    for (const overwrite of this.permissionOverwrites.values()) {
      if (overwrite.id === this.server.id) {
   ...
   * @param {ServerMember} member The member to obtain the overall permissions for
   * @returns {Readonly<Permissions>}
   * @private
   */
  memberPermissions(member) {
    if (member.id === this.server.ownerID) return new Permissions(Permissions.ALL).freeze();
  ...
  rolePermissions(role) {
    ...
    const everyoneOverwrites = this.permissionOverwrites.get(this.server.id);
  ...
  updateOverwrite(userOrRole, options, reason) {
    userOrRole = this.server.roles.resolve(userOrRole) || this.client.users.resolve(userOrRole);
  ...
  async edit(data, reason) {
    if (typeof data.position !== 'undefined') {
      await Util.setPosition(
        ...
        data.position,
        false,
        this.server._sortedChannels(this),
        this.client.api.servers(this.server.id).channels,
        reason,
    ...
    if (data.permissionOverwrites) {
      permission_overwrites = data.permissionOverwrites.map(o => PermissionOverwrites.resolve(o, this.server));
    }

    if (data.lockPermissions) {
      if (data.parentID) {
        const newParent = this.server.channels.resolve(data.parentID);
        if (newParent && newParent.type === 'category') {
          permission_overwrites = newParent.permissionOverwrites.map(o => PermissionOverwrites.resolve(o, this.server));
        }
      } else if (this.parent) {
        permission_overwrites = this.parent.permissionOverwrites.map(o => PermissionOverwrites.resolve(o, this.server));
      }
    }
  ...
  /**
   * Sets a new name for the server channel.
   * @param {string} name The new name for the server channel
   * @param {string} [reason] Reason for changing the server channel's name
   * @returns {Promise<ServerChannel>}
   * @example
  ...
  /**
   * Sets a new topic for the server channel.
   * @param {?string} topic The new topic for the server channel
   * @param {string} [reason] Reason for changing the server channel's topic
   * @returns {Promise<ServerChannel>}
   * @example
  ...
  /**
   * Sets a new position for the server channel.
   * @param {number} position The new position for the server channel
  ...
  /**
   * Creates an invite to this server channel.
  ...
  /**
   * Fetches a collection of invites to this server channel.
  ...
  clone(options = {}) {
    ...
    return this.server.channels.create(options.name, options);
  }
   ...
   * @param {ServerChannel} channel Channel to compare with
   * @returns {boolean}
   */
  equals(channel) {
    let equal =
