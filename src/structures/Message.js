...
class Message {
  ...
  /**
   * Whether or not this message has wumboji (larger emojis)
   * @type {boolean}
   * @readonly
   */
  get wumboji() {
    ...
    let matches = [];
    const emojiMatches = s.match(emojiRegex);
    const customMatches = s.match(/<:\w+:\d+>/g);
    if(emojiMatches) emojiMatches.map(m=>matches.push(m));
    if(customMatches) customMatches.map(m=>matches.push(m));
    if(!matches || matches.length > 27) return false; // Is above the 27 emoji limit, or no emojis.
    if(s.replace(emojiRegex, "").replace(/<:\w+:\d+>/g, "").match(/[^ \n]/g)) return false; // Has letters and stuff
    return true;
  }
