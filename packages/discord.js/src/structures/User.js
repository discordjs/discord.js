...
class User extends Base {
  ...
  /**
   * A link to the user's server tag badge.
   *
   * @param {ImageURLOptions} [options={}] Options for the image URL
   * @returns {?string}
   */
  serverTagBadgeURL(options = {}) {
    return (
      this.primaryGuild?.badge &&
      this.client.rest.cdn.serverTagBadge(this.primaryGuild.identityGuildId, this.primaryGuild.badge, options)
    );
  }
  ...
  toJSON(...props) {
    ...
    // TODO: Understand why bannerURL can be just the hash and see if the same should be done for serverTagBadgeURL
    json.serverTagBadgeURL = this.serverTagBadgeURL();
    return json;
  }
}
