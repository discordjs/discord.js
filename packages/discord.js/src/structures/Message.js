...
class Message extends Base {
  ...
  async fetchReference() {
    if (!this.reference) throw new Error(ErrorCodes.MESSAGE_REFERENCE_MISSING);
   ...
   */
  edit(options) {
    if (!this.channel) return Promise.reject(new Error(ErrorCodes.CHANNEL_NOT_CACHED));
   ...
   */
  async pin(reason) {
    if (!this.channel) throw new Error(ErrorCodes.CHANNEL_NOT_CACHED);
  ...
  startThread(options = {}) {
    ...
    if (![ChannelType.GuildText, ChannelType.GuildNews].includes(this.channel.type)) {
      return Promise.reject(new Error(ErrorCodes.MESSAGE_THREAD_PARENT));
    }
    if (this.hasThread) return Promise.reject(new Error(ErrorCodes.MESSAGE_EXISTING_THREAD));
   ...
   */
  fetchWebhook() {
    if (!this.webhookId) return Promise.reject(new Error(ErrorCodes.WEBHOOK_MESSAGE));
    if (this.webhookId === this.applicationId) return Promise.reject(new Error(ErrorCodes.WEBHOOK_APPLICATION));
