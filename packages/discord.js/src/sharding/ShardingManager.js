...
class ShardingManager extends EventEmitter {
  ...
  constructor(file, options = {}) {
     ...
     */
    this.file = file;
    if (!file) throw new Error(ErrorCodes.CLIENT_INVALID_OPTION, 'File', 'specified.');
    if (!path.isAbsolute(file)) this.file = path.resolve(process.cwd(), file);
    const stats = fs.statSync(this.file);
    if (!stats.isFile()) throw new Error(ErrorCodes.CLIENT_INVALID_OPTION, 'File', 'a file');
    ...
    if (this.shardList !== 'auto') {
      if (!Array.isArray(this.shardList)) {
        throw new TypeError(ErrorCodes.CLIENT_INVALID_OPTION, 'shardList', 'an array.');
      }
      this.shardList = [...new Set(this.shardList)];
      if (this.shardList.length < 1) {
        throw new RangeError(ErrorCodes.CLIENT_INVALID_OPTION, 'shardList', 'at least 1 id.');
      }
      if (
        this.shardList.some(
        ...
        throw new TypeError(ErrorCodes.CLIENT_INVALID_OPTION, 'shardList', 'an array of positive integers.');
      }
    }
    ...
    if (this.totalShards !== 'auto') {
      if (typeof this.totalShards !== 'number' || isNaN(this.totalShards)) {
        throw new TypeError(ErrorCodes.CLIENT_INVALID_OPTION, 'Amount of shards', 'a number.');
      }
      if (this.totalShards < 1) {
        throw new RangeError(ErrorCodes.CLIENT_INVALID_OPTION, 'Amount of shards', 'at least 1.');
      }
      if (!Number.isInteger(this.totalShards)) {
        throw new RangeError(ErrorCodes.CLIENT_INVALID_OPTION, 'Amount of shards', 'an integer.');
      }
    }
    ...
    if (this.mode !== 'process' && this.mode !== 'worker') {
      throw new RangeError(ErrorCodes.CLIENT_INVALID_OPTION, 'Sharding mode', '"process" or "worker"');
    }
  ...
  async spawn({ amount = this.totalShards, delay = 5500, timeout = 30_000 } = {}) {
    ...
    if (amount === 'auto') {
      ...
      if (amount < 1) throw new RangeError(ErrorCodes.CLIENT_INVALID_OPTION, 'Amount of shards', 'at least 1.');
      if (!Number.isInteger(amount)) {
        throw new TypeError(ErrorCodes.CLIENT_INVALID_OPTION, 'Amount of shards', 'an integer.');
      }
    }

    // Make sure this many shards haven't already been spawned
    if (this.shards.size >= amount) throw new Error(ErrorCodes.SHARDING_ALREADY_SPAWNED, this.shards.size);
  ...
  broadcastEval(script, options = {}) {
    if (typeof script !== 'function') return Promise.reject(new TypeError(ErrorCodes.SHARDING_INVALID_EVAL_BROADCAST));
  ...
  _performOnShards(method, args, shard) {
    if (this.shards.size === 0) return Promise.reject(new Error(ErrorCodes.SHARDING_NO_SHARDS));

    if (typeof shard === 'number') {
      if (this.shards.has(shard)) return this.shards.get(shard)[method](...args);
      return Promise.reject(new Error(ErrorCodes.SHARDING_SHARD_NOT_FOUND, shard));
    }

    if (this.shards.size !== this.shardList.length) return Promise.reject(new Error(ErrorCodes.SHARDING_IN_PROCESS));
