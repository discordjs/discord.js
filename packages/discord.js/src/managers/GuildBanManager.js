...
class GuildBanManager extends CachedManager {
   ...
   * @param {BanBulkOptions} [options] The options for bulk banning users
   * @returns {Promise<BanBulkResult>} Returns an object with `bannedUsers` key containing the IDs of the banned users
   * and the key `failedUsers` with the IDs that could not be banned or were already banned.
   * @example
   * // Bulk ban users by ids (or with user/guild member objects)
  ...
  async bulkCreate(users, options = {}) {
    if (!users || (!Array.isArray(users) && !(users instanceof Collection))) {
      throw new DiscordjsTypeError(ErrorCodes.InvalidType, 'users', 'array or collection', true);
    }
    if (typeof options !== 'object') throw new DiscordjsTypeError(ErrorCodes.InvalidType, 'options', 'object', true);
